# -*- coding: utf-8 -*-
"""
Created on Sun Mar 25 16:47:35 2018
...
def find_max_prod_in_grid(grid):
    max_prod = 0
    for row in xrange(len(grid)):
        for col in xrange(len(grid[0])):
            ...
            if col < len(grid[0]) - 3:
                test_prod = 1
                for i in xrange(4):
                    test_prod *= grid[row][col + i]
                if test_prod > max_prod:
                    max_prod = test_prod
                    print "horizontal, x:", col, "y:", row
            #sum vertically
            if row < len(grid) - 3:
                test_prod = 1
                for i in xrange(4):
                    test_prod *= grid[row + i][col]
                if test_prod > max_prod:
                    max_prod = test_prod
                    print "vertical, x:", col, "y:", row
            #sum diagonally downward
            if row < len(grid) - 3 and col < len(grid[0]) - 3:
                test_prod = 1
                for i in xrange(4):
                    test_prod *= grid[row + i][col + i]
                if test_prod > max_prod:
                    max_prod = test_prod
                    print "Diagonal Down, x:", col, "y:", row
            #sum diagonally upward
            if row > 3 and col < len(grid[0]) - 3:
                test_prod = 1
                for i in xrange(4):
                    test_prod *= grid[row - i][col + i]
                if test_prod > max_prod:
                    max_prod = test_prod
                    print "Diagonal Up, x:", col, "y:", row
    return max_prod
    
max_prod = find_max_prod_in_grid(grid)
print max_prod
